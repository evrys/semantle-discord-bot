    ...
    switch (interaction.data.name.toLowerCase()) {
      case GUESS_COMMAND.name.toLowerCase(): {
        if (interaction.data.type != ApplicationCommandType.ChatInput) {
          return new JsonResponse({ error: `Unexpected application command type ${interaction.data.type}` }, { status: 400 })
        }

        const words = []
        for (const opt of interaction.data.options || []) {
          if (opt.name === "word" && opt.type === ApplicationCommandOptionType.String) {
            words.push(opt.value)
          } else {
            return new JsonResponse({ error: `Unexpected option ${opt.name} of type ${opt.type}` }, { status: 400 })
          }
        }

        const word = words[0]
        if (!word || words.length !== 1) {
          return new JsonResponse({ error: `Expected one word, got ${words.length}` }, { status: 400 })
        }

        if (!interaction.member) {
          return new JsonResponse({ error: `No member in interaction` }, { status: 400 })
        }
        ...
        return {
          ...
          data: {
            content: await guessCommand(user, interaction.channel.id, word, env.KV)
          },
        }
      }
      ...
      case IGIVEUP_COMMAND.name.toLowerCase(): {
        ...
        if (confirm && confirm.type !== ApplicationCommandOptionType.String) {
          return new JsonResponse({ error: `Unexpected option type ${confirm.type}` }, { status: 400 })
        }
        ...
        const result = await giveupCommand(user, interaction.channel.id, env.KV)
